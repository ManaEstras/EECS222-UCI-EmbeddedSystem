...
behavior BlurX(in img image, in float kernel[WINSIZE], in int center,
		out float tempim[SIZE],out float kernel1[WINSIZE], out int center1)
{
...
behavior DUT(i_img_receiver ImgIn, i_img_sender ImgOut)
{
  ...
  //piped piped int center1;
	//piped piped float kernel1[WINSIZE];
  //piped piped short int delta_x1[SIZE];      /* The first devivative image, x-direction. */
	//piped piped short int delta_y1[SIZE];      /* The first derivative image, y-direction. */
	//piped piped short int magnitude1[SIZE];    /* The magnitude of the gadient image.      */
 
  piped int center1;
	piped float kernel1[WINSIZE];
  piped short int delta_x1[SIZE];      /* The first devivative image, x-direction. */
	piped short int delta_y1[SIZE];      /* The first derivative image, y-direction. */
	piped short int magnitude1[SIZE];    /* The magnitude of the gadient image.      */

	Gaussian_Smooth  gaussian_smooth(ImgIn, image, kernel, center);
	BlurX           blurX(image, kernel, center, tempim, kernel1, center1);
	BlurY           blurY(tempim, kernel1, center1, smoothedim);
