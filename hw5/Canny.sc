...
//#include <c_typed_double_handshack.sh>
...
DEFINE_C_TYPED_QUEUE(img, img)	// creates channel
...
behavior Stimulus(i_img_sender Outport){
  ...
  int count;
	...
	{ 
     ...
     if(read_pgm_image(infilename, imagein, ROWS, COLS) == 0){
...
behavior DataIn(i_img_receiver Inport, i_img_sender Outport){
	unsigned char imagein[SIZE];
	void main(void){
...
behavior Canny(i_img_receiver Inport,i_img_sender Outport){
	unsigned char imagein[SIZE];
	...
	for(count = 1 ; count < 21 ; count++)
 {
   ...
   canny(imagein, ROWS, COLS, SIGMA, TLOW, THIGH, edge);
...
behavior DataOut(i_img_receiver Inport, i_img_sender Outport){
	unsigned char edge[SIZE];
	void main(void){
	int count;
	for(count = 1; count <= 20 ; count ++)
...
behavior Monitor(i_img_receiver Inport)
  {
...
behavior Platform(i_img_receiver Inport,i_img_sender Outport)
{
	c_img_queue ImageInC(10ul);
	c_img_queue ImageOutC(10ul);
	DataIn DtIn(Inport,ImageInC);
	Canny DUT(ImageInC,ImageOutC);
	DataOut DtOut(ImageOutC,Outport);
	void main(void)
    {
	par { DtIn.main();
	      DUT.main();
...
behavior Main{
	c_img_queue IMGInC(10ul);
	c_img_queue IMGOutC(10ul);
	Stimulus Stim(IMGInC);
	Platform Plat(IMGInC,IMGOutC);
	Monitor Moni(IMGOutC);
	int main(void){
	par{
	Stim.main();
	Plat.main();
