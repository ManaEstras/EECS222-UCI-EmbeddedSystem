...
{
   char infilename[30] = "golfcart.pgm";  /* Name of the input image */
   char dirfilename[30] = "golfcartx.pgm"; /* Name of the output gradient direction image */
   char outfilename[128];    /* Name of the output "edge" image */
   char composedfname[128];  /* Name of the output "direction" image */
   unsigned char image[240][320];     /* The input image */
   unsigned char edge[240][320];      /* The output edge image */
         ...
         float tlow, float thigh, unsigned char edge[][320], char fname[])
{
   ...
   unsigned char nms[][320];        /* Points that are local maximal magnitude. */
   short int smoothedim[240][320],     /* The image after gaussian smoothing.      */
             delta_x[240][320],        /* The first devivative image, x-direction. */
             delta_y[240][320],        /* The first derivative image, y-direction. */
             magnitude[240][320];      /* The magnitude of the gadient image.      */
	...
	float tlow, float thigh, unsigned char edge[][320])
{
   ...
   ****************************************************************************/
   for(r=0,pos=0;r<rows;r++){
      for(c=0;c<cols;c++,pos++){
	 if(nms[r][c] == POSSIBLE_EDGE) edge[r][c] = POSSIBLE_EDGE;
	 else edge[r][c] = NOEDGE;
      }
   }
   ...
   ****************************************************************************/
   ...
   for(r=0,pos=0;r<rows;r++){
      for(c=0;c<cols;c++,pos++){
	 if(edge[r][c] == POSSIBLE_EDGE) hist[mag[r][c]]++;
      }
   }
   ...
   ****************************************************************************/
   for(r=0,pos=0;r<rows;r++){
      for(c=0;c<cols;c++,pos++){
	 if((edge[r][c] == POSSIBLE_EDGE) && (mag[r][c] >= highthreshold)){
            edge[r][c] = EDGE;
   ...
   ****************************************************************************/
   for(r=0,pos=0;r<rows;r++){
      for(c=0;c<cols;c++,pos++) if(edge[r][c] != EDGE) edge[r][c] = NOEDGE;
   }
}
...
* read in from either a file or from standard input. The image is only read
* from standard input when infilename = 0. Because the PPM format includes
* the number of columns and the number of rows in the image, these are read
* from the file. Memory to store the image is allocated in this function.
* All comments in the header are discarded in the process of reading the
* image. Upon failure, this function returns 0, upon sucess it returns 1.
